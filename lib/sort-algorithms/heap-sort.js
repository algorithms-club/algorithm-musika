...
let binaryHeap = {
	parent (ch) { return (Math.round(ch / 2) - 1); },
	child (p, ch) { return ((p + 1) * 2 - 2 + ch); },
	sibling (sib) { return (sib % 2 * 2 - 1 + sib); },
	build (arr) {
...
function checkUp (arr, childIndex) {
	let secChildIndex = binaryHeap.sibling(childIndex),
			parentIndex = binaryHeap.parent(childIndex),
...
function checkDown (arr, parentIndex, lastIndex = arr.length - 1) {
	let childIndex = binaryHeap.child(parentIndex, 1),
			secChildIndex = binaryHeap.child(parentIndex, 2),
			newParentIndex;
